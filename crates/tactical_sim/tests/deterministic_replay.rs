...
use tactical_sim::SimulationWorkBudget;
...
fn command_to_action_kind(command: &ActionKindSnapshot) -> ActionKind {
    match command {
        ...
        ActionKindSnapshot::Melee { target } => ActionKind::Melee {
            target: tactical_sim::ActorId::new(*target).expect("valid actor id"),
        },
        ActionKindSnapshot::PickUp { item } => ActionKind::PickUp {
            item: tactical_sim::ItemId::new(*item).expect("valid item id"),
        },
        ...
        ActionKindSnapshot::UseItem { item, target } => ActionKind::UseItem {
            item: tactical_sim::ItemId::new(*item).expect("valid item id"),
            target: match target {
                tactical_sim::persistence::snapshot::ActionTargetSnapshot::SelfTarget => {
                    tactical_sim::action::intent::ActionTarget::SelfTarget
                }
                tactical_sim::persistence::snapshot::ActionTargetSnapshot::Entity(id) => {
                    tactical_sim::action::intent::ActionTarget::Actor(
                        tactical_sim::ActorId::new(*id).expect("valid actor id"),
                    )
                }
                tactical_sim::persistence::snapshot::ActionTargetSnapshot::Cell { level, x, y } => {
                    tactical_sim::action::intent::ActionTarget::Cell {
                        level: LevelId(*level),
...
fn build_stable_entity_index(world: &mut World) {
    let mut index = tactical_sim::actor::components::StableEntityIndex::default();
...
fn initialize_world(app: &mut App, seed: u64) {
    ...
    let _loot = {
        let stable_item_id =
            tactical_sim::ItemId::new(loot_persistent_id.0).expect("valid item id");
        let entity = app.world_mut().spawn((
            Item,
...
fn configure_domain_work(app: &mut App, speed: SimSpeed, target: u64) {
    let mut driver = app
        .world_mut()
        .resource_mut::<tactical_sim::simulation::SimulationDriverState>();
...
fn drain_simulation_count(app: &mut App) -> usize {
    ...
    for _ in 0..4096 {
        ...
        let active_domain = app
            .world()
            .resource::<tactical_sim::simulation::SimulationDriverState>()
...
fn action_outcome_log_summary(app: &App) -> String {
    format!(
        "{:?}",
        app.world()
            .resource::<tactical_sim::action::resolver::ActionOutcomeLog>()
            .outcomes
    )
}
...
fn save_roundtrip_preserves_the_digest() {
    let fixture = load_fixture();
    let snapshot = run_replay(&fixture);
    let text = tactical_sim::persistence::snapshot::snapshot_to_text(&snapshot).expect("serialize");
...
fn snapshot_roundtrip_continues_a_partially_processed_simulation() {
    ...
    tactical_sim::drive_simulation(interrupted.world_mut());
    tactical_sim::drive_simulation(resumed.world_mut());
...
fn request_rejection_and_snapshot_request_window_invariants() {
...

    {
        ...
        assert!(matches!(
            ...
            Err(
                tactical_sim::simulation::DomainAdvanceError::TargetPrecedesClock {
                    clock_minute: 10,
                    requested_minute: 9
                }
            )
        ...
        assert!(matches!(
            ...
            Err(
                tactical_sim::simulation::DomainAdvanceError::RequestAlreadyActive {
                    active_target: 15,
                    requested_minute: 20
                }
            )
...
fn future_snapshot_versions_are_rejected() {
    ...
    assert!(
        migrate_snapshot(tactical_sim::persistence::migration::SnapshotFile::Current(
            snapshot
        ))
        .is_err()
    );
...
fn version_two_current_shape_is_upgraded() {
    ...
    let migrated = migrate_snapshot(tactical_sim::persistence::migration::SnapshotFile::Current(
        legacy_shape,
    ))
...
fn snapshot_roundtrip_preserves_non_tactical_driver_backlog() {
    ...
    let restored = match snapshot_from_text(&text).expect("deserialize") {
        tactical_sim::persistence::migration::SnapshotFile::Current(snapshot) => snapshot,
        tactical_sim::persistence::migration::SnapshotFile::V1(_) => {
            panic!("snapshot should not downgrade")
        }
        tactical_sim::persistence::migration::SnapshotFile::V2(_) => {
            panic!("snapshot should not downgrade")
        }
    };
...
fn continuation_after_restore_matches_the_original_world() {
    ...
    tactical_sim::persistence::snapshot::restore_world(
        restored_app.world_mut(),
        &restored_snapshot,
    )
...
fn snapshot_world_requires_authoritative_resources() {
    ...
    for (label, expected) in cases {
        ...
        match label {
            ...
            "effect queue" => {
                app.world_mut()
                    .remove_resource::<tactical_sim::item::effects::EffectQueue>();
            }
            "simulation driver" => {
                app.world_mut()
                    .remove_resource::<tactical_sim::simulation::SimulationDriverState>();
            }
            "simulation status" => {
            ...
            "action decision" => {
                app.world_mut()
                    .remove_resource::<tactical_sim::action::resolver::ActionDecision>();
            }
            _ => unreachable!(),
        }
...
fn failed_restore_does_not_mutate_the_live_world() {
    ...
    let result = tactical_sim::persistence::snapshot::restore_world(app.world_mut(), &corrupted);
...
fn malformed_snapshots_are_rejected() {
    ...
    for (label, snapshot, expected) in cases {
        let mut app = App::new();
        app.add_plugins(SimulationPlugin);
        let err = tactical_sim::persistence::snapshot::restore_world(app.world_mut(), &snapshot)
            .expect_err(label);
        assert!(
            err.contains(expected),
            "{} should mention `{}` but was `{}`",
    ...
    let _ = app.world_mut().run_system_once(
        |mut commands: Commands<'_, '_>, mut allocator: ResMut<'_, PersistentIdAllocator>| {
            ...
            commands.spawn((
                ...
                bonus_id,
                StableItemId(tactical_sim::ItemId::new(bonus_id.0).expect("valid item id")),
            ));
        },
    );
...
fn nonzero_level_ids_survive_restore_and_resave() {
    ...
    app.world_mut().spawn((
        ...
        player_id,
        StableActorId(tactical_sim::ActorId::new(player_id.0).expect("valid actor id")),
    ));
    ...
    let mut restored_app = App::new();
    restored_app.add_plugins(SimulationPlugin);
    tactical_sim::persistence::snapshot::restore_world(restored_app.world_mut(), &restored)
        .expect("restore");
...
fn apply_pending_effects_batches_statuses_and_persists_them() {
    ...
    app.world_mut()
        ...
        .push_back(Effect::ApplyStatus {
            target: tactical_sim::ActorId::new(player_id.0).expect("valid actor id"),
...
fn select_next_actor_preserves_scheduled_work_when_the_index_is_stale() {
    ...
    let actor_id = tactical_sim::ActorId::new(1).expect("valid actor id");
    let entity = app
        .world_mut()
        .spawn((
            Actor,
    ...
    app.world_mut()
        .run_system_once(tactical_sim::time::scheduler::select_next_actor)
...

    {
        let mut index = app
            .world_mut()
            .resource_mut::<tactical_sim::actor::components::StableEntityIndex>();
...
fn spatial_index_orders_occupants_by_stable_identity() {
    ...
    let first = app
        ...
        .spawn((
            ...
            StableActorId(tactical_sim::ActorId::new(2).expect("valid actor id")),
        ))
        .id();
    let second = app
        .world_mut()
